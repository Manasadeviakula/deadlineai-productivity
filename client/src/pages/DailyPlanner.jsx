...
export const DailyPlanner = () => {
  ...
  const fetchDailySchedule = async () => {
    ...
    try {
      ...
      const res = await api.post('/ai/daily-plan', { currentTime });
      if (res.data.success) {
    ...
    <div className="space-y-8 max-w-5xl mx-auto">
      ...
      {loading ? (
        <div className="py-20 text-center glass-panel rounded-3xl space-y-4">
          ...
          <p className="text-base font-semibold text-slate-200">Synthesizing optimized timeline...</p>
        </div>
      ) : scheduleData ? (
        <div className="space-y-6">
          
          {/* Productivity Forecast Header */}
          <div className="glass-panel p-6 rounded-3xl bg-gradient-to-r from-indigo-900/40 via-purple-900/40 to-slate-900/40 border border-indigo-500/30 flex items-center justify-between">
            <div>
              <span className="text-xs font-semibold uppercase tracking-wider text-indigo-400">Forecasted Daily Output</span>
              <h2 className="text-2xl font-bold text-white mt-1">{scheduleData.productivityScoreForecast}% Target Score</h2>
            </div>
            <div className="p-4 rounded-2xl bg-indigo-500/20 text-indigo-300 font-bold text-xl">
              🎯 Optimized
            </div>
          </div>
          ...
          <div className="glass-panel p-6 rounded-3xl space-y-4 relative">
            <h3 className="text-sm font-semibold uppercase tracking-wider text-slate-400 mb-4">Today's Timeline Blocks</h3>
            
            <div className="space-y-4 relative before:absolute before:inset-0 before:left-6 before:w-0.5 before:bg-slate-700/50">
                  ...
                  <motion.div
                    ...
                    className={`relative pl-12 p-4 rounded-2xl border transition-colors ${
                      isBreak 
                        ...
                        : 'bg-slate-800/60 border-slate-700/60 hover:border-indigo-500/40 text-slate-100'
                    }`}
                  >
                    <div className={`absolute left-3.5 top-5 h-5 w-5 rounded-full border-2 flex items-center justify-center text-xs font-bold ${
                      isBreak ? 'bg-amber-500 border-amber-400 text-slate-950' : 'bg-indigo-600 border-indigo-400 text-white'
                    }`}>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                      <div>
                        ...
                        <h4 className="font-bold text-base mt-0.5">{item.activity}</h4>
                      </div>

                      <span className={`px-3 py-1 rounded-xl text-xs font-semibold self-start sm:self-auto ${
                        isBreak ? 'bg-amber-500/20 text-amber-300' : 'bg-indigo-500/20 text-indigo-300'
                      }`}>
          ...
          {scheduleData.tips && (
            <div className="glass-panel p-6 rounded-3xl bg-slate-900/60 border border-slate-800">
              <h4 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">💡 Mentor Tips for Today</h4>
